...
for run in tqdm.tqdm(os.listdir(fname), "Bags"):
    ...
    if not split_bags:
        ...
        try:
            pgo_info_dict = yaml.safe_load(rosbag.Bag(bag_location, 'r')._get_yaml_info())
        ...
        pg_info_dict = yaml.safe_load(rosbag.Bag(pose_graph_bag, 'r')._get_yaml_info())
        print(pg_info_dict)
        pgo_info_dict = yaml.safe_load(rosbag.Bag(pose_graph_opt_bag, 'r')._get_yaml_info())
        print(pgo_info_dict)
        last_time = None        
